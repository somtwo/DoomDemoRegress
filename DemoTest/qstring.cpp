...
qstring &qstring::createSize(size_t pSize)
{
	...
	else
	{
		...
		if (size < pSize)
		{
			buffer = (char *)realloc(buffer, sizeof(char) * pSize);
			size = pSize;
		}
		clear();
	}
...
{
	assert(idx < size, "qstring::charAt: index out of range\n");
...
{
	assert(idx < size, "qstring::operator []: index out of range\n");
...
qstring &qstring::insert(const char *insertstr, size_t pos)
{
	...
	assert(pos < index, "qstring::insert: position out of range\n");
...
qstring &qstring::truncate(size_t pos)
{
	...
	assert(pos < index, "qstring::truncate: position out of range\n");
...
qstring &qstring::erase(size_t pos, size_t n)
{
	...
	// pos must be between 0 and qstr->index - 1
	assert(pos < index, "qstring::erase: position out of range\n");
...
{
	return _strnicmp(buffer, str, maxcount);
}
...
{
	assert(false, "qstring::HashCodeStatic method not implemented");
	return 0;
}
...
{
	assert(false, "qstring::HashCodeCaseStatic method not implemented");
	return 0;
}
...
{
	assert(false, "qstring::findSubStrNoCase method not implemented");
	return NULL;
}
...
size_t qstring::find(const char *s, size_t pos) const
{
	...
	assert(pos < index, "qstring::find: position out of range\n");
...
{
	assert(false, "qstring::duplicateAuto method not implemented");
	return NULL;
}
...
static char *qstr_repltable[256];
...
{
	assert(false, "qstring::replace method not implemented.");
...
{
	assert(false, "qstring::normalizeSlashes is not implemented");
...
int qstring::Printf(size_t maxlen, const char *fmt, ...)
{
	...
	va_start(va2, fmt);
	returnval = vsnprintf(buffer, size, fmt, va2);
	va_end(va2);
